...
static jint nativeConnect(JNIEnv *env, jobject thiz,
	...
	jint busNum, jint devAddr, jstring usbfs_str, jint videoIndex) {
...
//		libusb_set_debug(NULL, LIBUSB_LOG_LEVEL_DEBUG);
		result =  camera->connect(vid, pid, fd, busNum, devAddr, c_usbfs, videoIndex);
	}
...
static jint nativeSetCameraRotationDegrees(JNIEnv *env, jobject thiz,
										   ID_TYPE id_camera, jint cameraRotationDegrees) {

	jint result = JNI_ERR;
	ENTER();
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->setCameraRotationDegrees(cameraRotationDegrees);
	}
	RETURN(result, jint);
}
...
//	{ "nativeGetFocusSimple",			"(J)I", (void *) nativeGetFocusSimple },
	...
	{ "nativeSetCameraRotationDegrees",	"(JI)I", (void *) nativeSetCameraRotationDegrees }
};
