...
//				uint32_t value = 0x00003201; //小端，需要采用小端  0x16 表示亮度（0x64最亮 0x00熄灭） 0x01 标识哪种灯
				...
				int ret = uvc_set_ctrl(mDeviceHandle, unit_id, control_id, &value, sizeof(value));
...
int UVCCamera::setCameraRotationDegrees(int rotationDegrees) {
	ENTER();
	int result = EXIT_FAILURE;
	if (mDeviceHandle) {
		const uvc_extension_unit *xus = uvc_get_extension_units(mDeviceHandle);
		do {
			...
			if (uintid == 0x06) {
				uint8_t unit_id = uintid;
				uint8_t control_id = 1;
//				int value = 0x00003201; //小端，需要采用小端
				if (rotationDegrees == 0) {
					rotationDegrees = 0x00;
				} else if (rotationDegrees == 90) {
					rotationDegrees = 0x01;
				} else if (rotationDegrees == 180) {
					rotationDegrees = 0x02;
				} else if (rotationDegrees == 270) {
					rotationDegrees = 0x03;
				} else {
					rotationDegrees = 0x00;
				}
				uint32_t value = 0x10 + (rotationDegrees << 8);
				int ret = uvc_set_ctrl(mDeviceHandle, unit_id, control_id, &value, sizeof(value));
				LOGE("uvc_set_ctrl ret=%d sizeof(value)=%d", ret, sizeof(value));
				break;
			}
